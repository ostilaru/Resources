...
void calFilterResp(double *b, double *a, int nspec, complex *response)
{
    for (int i = 0; i < nspec + 1; i++)
    {
        float normalized_freq = i / nspec;
...
FilterResp *processButterworthFilters(ButterworthFilter *filters, int filterCount, float df, int nspec)
{
    ...
    for (int i = 0; i < filterCount; i++)
    {
        ...
        if (responses[i].response != NULL)
        {
            memset(responses[i].response, 0, nfft_2x * sizeof(complex));
        }

        // calculate filter response
        calFilterResp(filters[i].b, filters[i].a, nspec, responses[i].response);
    }
    return responses;
}