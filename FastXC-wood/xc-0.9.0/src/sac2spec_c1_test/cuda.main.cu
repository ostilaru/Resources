...
int main(int argc, char **argv)
{
    ...
    FilterResp *myResp = processButterworthFilters(filter, filter_count, df_1x, nspec_1x);
    ...
    for (int i = 0; i < filter_count; i++)
    {
        ...
        CUDACHECK(cudaMemcpy2D(d_filter_responses, nfft_1x * sizeof(cuComplex), myResp[i].response, nfft_1x * sizeof(cuComplex),
        ...
        for (size_t d_finishcnt = 0; d_finishcnt < h_proccnt; d_finishcnt += d_batch)
        {
            ...
            for (int stepidx = 0, done_step_flag = 0; stepidx < nstep; stepidx++)
            {
                ...
                if (wh_before || do_runabs)
                {
                    ...
                    if (wh_before)
                    {
                        freqWhiten(d_segment_spectrum, d_weight, d_tmp, nfft_1x, nspec_1x, d_proccnt, nsmooth, df_1x, freq_low_limit, freq_high_limit, FILTERFLAG);
                    }
                ...
                if (do_onebit)
                {
                    onebit(d_segment_timesignal, nfft_1x, d_proccnt);
                    CUFFTCHECK(cufftExecR2C(planfwd, (cufftReal *)d_segment_timesignal, (cufftComplex *)d_segment_spectrum));
                    FwdNormalize2DKernel<<<dimgrd_1x, dimblk_1x>>>(d_segment_spectrum, nfft_1x, nfft_1x, d_proccnt, delta);
                }
