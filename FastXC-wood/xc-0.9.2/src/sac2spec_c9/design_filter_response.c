...
void calFilterResp(double *b, double *a, int nfft_1x, complex *response)
{
    int i, j;
    for (i = 0; i < nfft_1x/2 + 1; i++)
    {
        float normalized_freq = (float)i / (float)(nfft_1x / 2 + 1);
...
FilterResp *processButterworthFilters(ButterworthFilter *filters, int filterCount, float df_1x, int nfft_1x)
{
    ...
    for (i = 0; i < filterCount; i++)
    {
        ...
        responses[i].response = (complex *)malloc(nfft_1x * sizeof(complex));
        ...
        if (responses[i].response != NULL)
        {
            memset(responses[i].response, 0, nfft_1x * sizeof(complex));
        }

        // calculate filter response
        calFilterResp(filters[i].b, filters[i].a, nfft_1x, responses[i].response);
    }
    return responses;
}